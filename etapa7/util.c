...
lexic_val_type* createVal(int tipo_token, int tipo_lit, int lineno, char* yytext)
{
...
lexic_val_type* invVal( lexic_val_type* lexValue)
{
	...
	{
		case(LIT_TIPO_INT):
			lexValue->tk_value.vInt = - lexValue->tk_value.vInt; 
			break;
		case(LIT_TIPO_FLOAT):
			lexValue->tk_value.vFloat = - lexValue->tk_value.vFloat ;
			break;
		default:
			break;
	}
